...
void sample_input_wave(void)
{
	// << TODO >> : Complete the logic in function as expected in the comment above.
		...
		for (int i = 0; i < 192; i++)
		{
			...
			else if(ms_per_div == 0.5){
				_delay_us(60);
			}
			else if(ms_per_div == 0.624){
				_delay_us(82);
			}
			else if(ms_per_div == 1){
				_delay_us(123);
			}
			else if(ms_per_div == 1.124){
				_delay_us(135);
			}
			else if(ms_per_div == 1.5){
				_delay_us(180);
			}
			else if (ms_per_div == 1.624)
			{
				_delay_us(200);
			}
			else if (ms_per_div == 2)
			{
				_delay_us(244);
			}
			//_delay_us(60);
...
void draw_waveform(void)
{
	...
	for (int x = 0; x < 95; x++)
	{
		y1 = pixel_loc[x] + (vert_disp/2);				// move the waveform up and down by +/- vert_disp
		y2 = pixel_loc[x + 1] + (vert_disp/2);			// move the waveform up and down by +/- vert_disp
